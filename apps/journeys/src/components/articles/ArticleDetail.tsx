  ...
  return (
    <article>
      <section className="relative min-h-[min(62vh,680px)] w-full overflow-hidden">
        ...
        <div className="relative z-10 flex min-h-[min(62vh,680px)] flex-col justify-end pb-[var(--space-12)] pt-28">
          <div className="mx-auto w-full max-w-6xl px-[clamp(1.5rem,5vw,4rem)]">
            <p className="mb-[var(--space-3)] text-sm uppercase tracking-wider text-white/70">Article</p>
            <h1 className="max-w-4xl font-display text-4xl font-bold tracking-tight text-white sm:text-5xl md:text-6xl">
              {article.title}
            </h1>
            {article.excerpt ? (
              <p className="mt-[var(--space-4)] max-w-2xl text-lg text-white/60 sm:text-xl">{article.excerpt}</p>
            ) : null}
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto w-full max-w-6xl px-[clamp(1.5rem,5vw,4rem)] py-[var(--space-12)] md:py-[var(--space-16)]">
        ...
        {article.gallery && article.gallery.length > 0 ? (
          <section className="mt-[var(--space-16)]" aria-labelledby="article-gallery-heading">
            <h2
